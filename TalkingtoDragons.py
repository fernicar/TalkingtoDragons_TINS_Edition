...
Your entire output must be a single, refined text-to-image prompt only on a single text line. Do not provide any conversational text, explanations, or reasoning. The output should be ready for direct use with the Flux Dev model."""
    ...
    if is_enhancement:
        ...
        full_prompt = f"{system_prompt}\n\nCreate a new prompt based on this theme/style: {base_prompt}"
    ...
    while retries < max_retries:
        try:
            request_json = {
                "model": model_var.get(),
                "prompt": full_prompt,
                "max_tokens": 400,  # Enough for 250-300 words
                "temperature": 0.7,
                "top_p": 0.85,
                "stream": True
            }
            ...
            # Accept if in range and complete
            if 250 <= word_count <= 300 and is_complete:
            ...
            if retries < max_retries - 1:
                retries += 1
                if word_count < 250:
                    full_prompt = f"{system_prompt}\n\nInput prompt to enhance: {base_prompt}\n\nContinue from: {clean_prompt}\nExpand this to reach 250-300 words while maintaining single line format."
                elif not is_complete:
                    full_prompt = f"{system_prompt}\n\nInput prompt to enhance: {base_prompt}\n\nComplete this prompt properly: {clean_prompt}"
                else:
                    break  # Too long but complete - accept it
            else:
